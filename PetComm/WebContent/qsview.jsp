...
<%
	...
	Connection conn3 = null;
	 ...
	 try{
	 	...
	 	String sql3 = "select name FROM qsgood where uid=? and name=?";
...
<%
 ...
 try{
 	...
 	conn1 = DriverManager.getConnection(jdbcUrl,dbId,dbPass);
 		...
 		String sql1 = "select name,subject,comment,signdate,ref,img FROM reboard where uid=?";
...
<%
 ...
 try{
 	...
 	conn = DriverManager.getConnection(jdbcUrl,dbId,dbPass);
 		...
 		String sql = "update  reboard  set ref = ?  where uid = ?";
 ...
 <strong><h1><a href="index.jsp?target=qsboard"><font color="black">질문게시판</font></a></h1></strong>
   	...
   	<tr>
   		<td align="center" style="padding-left: 300px;">
   			<nav>
  				<ul class="pagination">
    				<li id="good">
    					<a href="index.jsp?target=goodaction&number=<%= number %>&what=qs">
    						<% 
		...
		<td width="20%" align="right">
 		...
 		if(name.equals(who))
 		{
 			...
 			<nav>
 	  			<ul class="pagination">
 	    			<li><a href="index.jsp?target=updateform&number=<%= number %>&lang=<%= lang%>&what=qs">수정</a></li>
 	    			<li><a href="index.jsp?target=deleteaction&number=<%= number %>&lang=<%= lang%>&what=qs">삭제</a></li>
 	  			</ul>
 			</nav>
 			<%
 		}
...
<%
	if(session.getAttribute("ss") == null)
	{
		...
		<form class="form-inline" role="form"  method="post" action="index.jsp?target=repost&lang=<%=lang%>&number=<%= number%>">
  			<div class="form-group">
    			<textarea name="comment" class="form-control" cols="100" rows="400" style="height: 50px;" placeholder="댓글을 작성하려면 로그인이 필요합니다."></textarea>
  			</div>
	...
	else 
	{
		...
		<br><br><br><br>
		<form class="form-inline" role="form" id="repost" name="repost"  method="post"  onsubmit="return blank();" action="index.jsp?target=repost&lang=<%=lang%>&number=<%= number%>" >
  			<div class="form-group">
    			<textarea name="comment" class="form-control" cols="100" rows="400" style="height: 50px;"></textarea>
  			</div>
...
<%
 ...
 try{
 	...
 	String sql2 = "select name,comment,uid from replylist where fid=? order by uid desc";
 		...
 		<tr>
 			...
 			<td align="right">
 				<%
 					...
 					{
 						%>
 							<form class="form-horizontal" role="form" method="post" action="index.jsp?target=replydelete&uid=<%= uid%>&number=<%= number %>&what=qs">
									<div class="form-group">

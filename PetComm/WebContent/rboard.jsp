...
%>
<body>
<strong><h1><% if(lang.equals("korea")){%>리뷰게시판<%}else if(lang.equals("china")){%>写作 <%}else if(lang.equals("america")){%>Notice board<%}   %></h1></strong>
<div align="right">
<nav>
  <ul class="pagination">
    <li><a href="index.jsp?target=postform&lang=<%= lang%>"><% if(lang.equals("korea")){%>글 올리기<%}else if(lang.equals("china")){%>写作 <%}else if(lang.equals("america")){%>Write<%}   %></a></li>
  </ul>
</nav>
</div>
<%
...
try{
	...
	String sql1 = "select uid,name,subject,signdate,ref from freeboard order by uid desc limit ?,?";
		...
		<tr style="background-color: #FCF8E3">
		 ...
		 try{
		 	...
		 	conn6 = DriverManager.getConnection(jdbcUrl,dbId,dbPass);
		 		...
		 		String sql6 = "select uid,name,subject,signdate,ref from freeboard order by good desc limit 2";
		 		...
		 		while(rs6.next())
		 		{
		 			...
		 			%>
		 				<tr>
		 					...
		 					<td>
		 						<a href="index.jsp?target=view&number=<%= best_uid%>&lang=<%= lang%>"><%= best_subject %>
		 						<%
 ...
 try{
 	...
 	conn1 = DriverManager.getConnection(jdbcUrl,dbId,dbPass);
 		...
 		String sql2 = "select count(comment) FROM reviewreplylist where fid=?";
 		...
 		sql1 = "select count(uid) FROM freegood where uid=?";
 ...
 %>
	...
	while(rs.next())
	{
		...
		<tr>
 	 		<td><%=article_num %></td>
 	 		<td>
 	 			<a href="index.jsp?target=view&number=<%= uid%>&lang=<%= lang%>"><%=subject %>
...
{
	 %>
	 <li><a href="index.jsp?target=rboard&pg=<%= prevPage %>&lang=<%= lang%>"><% if(lang.equals("korea")){%>이전<%}else if(lang.equals("china")){%>以前 <%}else if(lang.equals("america")){%>back<%}   %></a></li>
	 <%
}
%>
<%
for(int i= 1 + prevPage; i < nextPage && i<= totalPage; i++)
{
	...
	else
	{
		%>
    			...
    			<li><a href="index.jsp?target=rboard&pg=<%= i %>&lang=<%= lang%>"><%= i %></a></li>
...
{
	%>
	<li><a href="index.jsp?target=rboard&pg=<%= nextPage %>&lang=<%= lang%>"><% if(lang.equals("korea")){%>다음<%}else if(lang.equals("china")){%>下一个 <%}else if(lang.equals("america")){%>next<%}   %></a></li>
	<%
}
%>
</ul>

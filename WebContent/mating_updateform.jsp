...
<%
 ...
 try{
 	...
 	conn = DriverManager.getConnection(jdbcUrl,dbId,dbPass);
 		...
 		sql = "select name,subject,comment FROM mating where uid=?";
 ...
 %>
<form class="form-horizontal" role="form"  action="index.jsp?target=mating_updateaction&lang=<%=lang%>&what=<%=what%>&number=<%= number %>"  method="post" enctype="Multipart/form-data" >
	...
	<div class="form-group">
		...
		<input type="hidden" name="number" value="<%=number %>">
			<button type="submit" class="btn btn-default"><% if(lang.equals("korea")){%>수정하기<%}else if(lang.equals("china")){%>修整 <%}else if(lang.equals("america")){%>Modify<%}   %></button> 
				<a href="index.jsp?target=qsmatingview&lang=<%=lang%>&number=<%= number %>"><button type="button" class="btn btn-default"><% if(lang.equals("korea")){%>취소<%}else if(lang.equals("china")){%>取消 <%}else if(lang.equals("america")){%>Cancel<%}   %></button> </a>

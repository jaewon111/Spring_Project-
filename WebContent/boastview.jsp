...
#good
{
	...
	Connection conn3 = null;
	 ...
	 try{
	 	...
	 	String sql3 = "select name FROM boastgood where uid=? and name=?";
...
<%
 ...
 try{
 	...
 	conn1 = DriverManager.getConnection(jdbcUrl,dbId,dbPass);
 		...
 		String sql1 = "select name,subject,comment,signdate,ref,img FROM boast where uid=?";
...
<%
 ...
 try{
 	...
 	conn = DriverManager.getConnection(jdbcUrl,dbId,dbPass);
 		...
 		String sql = "update  boast  set ref = ?  where uid = ?";
...
<strong><h1><a href="index.jsp?target=boast"><font color="black">자랑게시판</font></a></h1></strong>
 <br><br><br>
  		...
  		<td colspan="2">
   			<%
   				...
   				<%
   	   			...
   	   			{
   	   			%>
   	   				<img src="uploadfiles/<%=img %>" ></img><br><%= comment %>
   	   			<%} %>
   	...
   	<tr>
   		<td align="center" style="padding-left: 300px;">
   			<nav>
  				<ul class="pagination">
    				<li id="good">
    					<a href="index.jsp?target=goodaction&number=<%= number %>&what=boast">
    						<% 
    						if(compare.equals(who))
    						{
    							%>
		...
		<td width="20%" align="right">
			<%
 			if(name.equals(who))
 			{
 				...
 				<nav>
 	  				<ul class="pagination">
 	    				<li><a href="index.jsp?target=updateform&number=<%= number %>&lang=<%= lang%>&what=boast"><% if(lang.equals("korea")){%>수정<%}else if(lang.equals("china")){%>修整 <%}else if(lang.equals("america")){%>Modify<%}   %></a></li>
 	    				<li><a href="index.jsp?target=deleteaction&number=<%= number %>&lang=<%= lang%>&what=boast"><% if(lang.equals("korea")){%>삭제<%}else if(lang.equals("china")){%>删除 <%}else if(lang.equals("america")){%>Delete<%}   %></a></li>
 	  				</ul>
 				</nav>
 				<%
 			}
...
<%
	if(session.getAttribute("ss") == null)
	{
		%>
		<br><br><br><br>
		<form class="form-inline" role="form"  method="post" action="index.jsp?target=repost&lang=<%=lang%>&number=<%= number%>">
	...
	else
	{
		%>
		<br><br><br><br>
		<form class="form-inline" role="form"  method="post" action="index.jsp?target=boastrepost&number=<%= number%>">
  			<div class="form-group">
...
<%
 ...
 try{
 	...
 	String sql2 = "select name,comment,uid from boastreplylist where fid=? order by uid desc";
 	...
 	while(rs2.next())
 	{
 		...
 		//sql2 = "select name from member where id=?";
 	 	//pstmt2 = conn2.prepareStatement(sql2);
 	 	//pstmt2.setString(1, replywriter);
 	 	//rs2 = pstmt2.executeQuery();
 	 	//while(rs2.next())
 	 	//{
 		...
 		%>
 		<tr>
 			<td><%=nickname %></td><td><%=reply %></td>
 			<td align="right">
 				<%
 					if(replywriter.equals(who))
 					{
 						%>
 							<form class="form-horizontal" role="form" method="post" action="index.jsp?target=replydelete&uid=<%= uid%>&number=<%= number %>&what=boast">
									<div class="form-group">

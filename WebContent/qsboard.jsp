...
%>
<body>
<strong><h1><% if(lang.equals("korea")){%>질문게시판<%}else if(lang.equals("china")){%>写作 <%}else if(lang.equals("america")){%>Notice board<%}   %></h1></strong>
<div align="right">
<nav>
  <ul class="pagination">
    <li><a href="index.jsp?target=qsboardform&lang=<%= lang%>"><% if(lang.equals("korea")){%>글 올리기<%}else if(lang.equals("china")){%>写作 <%}else if(lang.equals("america")){%>Write<%}   %></a></li>
  </ul>
</nav>
</div>
<%
...
try{
	...
	String sql = "select count(uid) from reboard";
	String sql1 = "select uid,name,subject,signdate,ref from reboard order by uid desc limit ?,?";
		...
		<%
		 ...
		 try{
		 	...
		 	conn6 = DriverManager.getConnection(jdbcUrl,dbId,dbPass);
		 		...
		 		String sql6 = "select uid,name,subject,signdate,ref from reboard order by good desc limit 2";
		 		...
		 		while(rs6.next())
		 		{
		 			...
		 			%>
		 				<tr>
		 					...
		 					<td>
		 						<a href="index.jsp?target=qsview&number=<%= best_uid%>&lang=<%= lang%>"><%= best_subject %>
		 						<%
 ...
 try{
 	...
 	conn1 = DriverManager.getConnection(jdbcUrl,dbId,dbPass);
 		...
 		String sql2 = "select count(comment) FROM replylist where fid=?";
 		...
 		sql1 = "select count(uid) FROM qsgood where uid=?";
 ...
 %>
	...
	while(rs.next())
	{
		...
		<tr>
 	 		<td><%=article_num %></td>
 	 		<td><a href="index.jsp?target=qsview&number=<%= uid%>&lang=<%= lang%>"><%=subject %>
...
{
	 %>
	 <li><a href="index.jsp?target=qsboard&pg=<%= prevPage %>&lang=<%= lang%>"><% if(lang.equals("korea")){%>이전<%}else if(lang.equals("china")){%>以前 <%}else if(lang.equals("america")){%>back<%}   %></a></li>
	 <%
}
%>
<%
for(int i= 1 + prevPage; i < nextPage && i<= totalPage; i++)
{
	...
	{
		%>
		<li><a href="index.jsp?target=qsboard&pg=<%= i %>&lang=<%= lang%>"><%= i %></a></li>
		<%
	}
}
%>
...
{
	%>
	<li><a href="index.jsp?target=qsboard&pg=<%= nextPage %>&lang=<%= lang%>"><% if(lang.equals("korea")){%>다음<%}else if(lang.equals("china")){%>下一个 <%}else if(lang.equals("america")){%>next<%}   %></a></li>
	<%
}
%>

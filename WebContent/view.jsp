...
<%
	...
	Connection conn3 = null;
	 ...
	 try{
	 	...
	 	String sql3 = "select name FROM freegood where uid=? and name=?";
...
<%
 ...
 try{
 	...
 	conn1 = DriverManager.getConnection(jdbcUrl,dbId,dbPass);
 		...
 		String sql1 = "select name,subject,comment,signdate,ref,img FROM freeboard where uid=?";
...
<%
 ...
 try{
 	...
 	conn = DriverManager.getConnection(jdbcUrl,dbId,dbPass);
 		...
 		String sql = "update  freeboard  set ref = ?  where uid = ?";
...
<strong><h1><a href="index.jsp?target=rboard"><font color="black">리뷰게시판</font></a></h1></strong>
 <br><br><br>
 ...
 <tr>
   		<td align="center" style="padding-left: 300px;">
  				...
  				<ul class="pagination">
    				<li id="good">
    					<a href="index.jsp?target=goodaction&number=<%= number %>&what=free">
    						<% 
		...
		<td width="20%" align="right">
 		...
 		if(name.equals(who))
 		{
 			...
 			<nav>
 	  			<ul class="pagination">
 	    			<li><a href="index.jsp?target=updateform&number=<%= number %>&lang=<%= lang%>&what=free">수정</a></li>
 	    			<li><a href="index.jsp?target=deleteaction&number=<%= number %>&lang=<%= lang%>&what=free">삭제</a></li>
 	  			</ul>
 			</nav>
 			<%
 		}
...
<%
	if(session.getAttribute("ss") == null)
	{
		%>
		<br><br><br><br>
		<form class="form-inline" role="form"  method="post"  action="index.jsp?target=reviewreplypost&lang=<%=lang%>&number=<%= number%>">
	...
	else 
	{
		...
		<br><br><br><br>
		<form class="form-inline" role="form" id="repost" name="repost" method="post" onsubmit="return blank();" action="index.jsp?target=reviewreplypost&lang=<%=lang%>&number=<%= number%>">
  			<div class="form-group">
...
<%
 ...
 try{
 	...
 	String sql2 = "select name,comment,uid from reviewreplylist where fid=? order by uid desc";
 		...
 		<tr>
 			...
 			<td align="right">
 				<%
 					...
 					{
 						%>
 							<form class="form-horizontal" role="form" method="post" action="index.jsp?target=replydelete&uid=<%= uid%>&number=<%= number %>&what=review">
									<div class="form-group">
